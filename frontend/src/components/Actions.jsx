...
function Actions({ post }) {
  ...
  const handleLikeAndUnlike = async () => {
    ...
    try {
      ...
      if (!liked) {
        //add the id of the user to the post.likes array
        ...
        //remove the id of the user to the post.likes array
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
            return {
              ...p,
  ...
  const handleReply = async () => {
    ...
    if (!post) return showToast("Error", "No post available", "error");
    setIsReplying(true);
    if (!user) return showToast("Error", "Login to reply a post", "error");
    try {
      // console.log(post);
  ...
  if (!post) return <h1>not getting posts data</h1>;
  return (
